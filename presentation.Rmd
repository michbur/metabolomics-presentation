...
Enumerate all possible fragments of candidate structures by systematic bond cleavage. Fragmentation tree (FT) is computed directly from an MS$^2$ spectrum.
...
dat <- rbind(data.frame(x = 1L:5, y = 1L:5 + rnorm(5, 0.02),
                        region = "Interpolation"),
             data.frame(x = 6L:10, y = (6L:10*2.5 + rnorm(5, 0.02)),
                        region = "Extrapolation"))

p1 <- filter(dat, region == "Interpolation") %>% 
  ggplot(aes(x = x, y = y)) +
  geom_point() 
...
```{r}
p1 + geom_smooth(method = "lm") + ggtitle("Interpolation")
```
...
p1 +
  ...
  geom_point(data = dat, aes(color = region)) + 
...
As lipids are based on fundamental building blocks, their fragmentation leads to reoccurring and fairly predictive patterns. The is to choose the fragmentation tree that best explains the observed data.

E.g., SIRIUS.

## Heuristic-based methods

Fragmentation patterns are constructed utilizing so-called decision rulesets that are tested against experimental spectra.

E.g., Lipid Data Analyzer, LipidXplorer.

## Machine learning approaches

Supervised: FingerID, DeepEI, CSI:FingerID.

Unsupervised: MS2LDA, Word2Vec.
